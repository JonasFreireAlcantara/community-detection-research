...
from common.loader.jonas_si_eon_bp_loader import JonasSIEONBlockingProbabilityLoader
...
class EONBlockingProbabilitySI:
    ...
    def load_and_save_gml_html(self, save_html=False):
        ...
        for k in range(9, len(lines), 10):
            line = lines[k]
            graph = JonasSIEONBlockingProbabilityLoader(self.gml_path, line).parse()
...
if __name__ == '__main__':
    ...
    files = [
        'dataset_arnes_shuffle_0_eon.txt',
        'dataset_arnes_shuffle_1_eon.txt',
        'dataset_arnes_shuffle_2_eon.txt',
        'dataset_arnes_shuffle_3_eon.txt'
    ]
    counter = 0
    for file in files:
        file_input = os.path.join(ResourcesConstants.ARNES_EON_BP, file)
        resilience_si = EONBlockingProbabilitySI(gml_path, file_input, target_directory, file_prefix=counter)
