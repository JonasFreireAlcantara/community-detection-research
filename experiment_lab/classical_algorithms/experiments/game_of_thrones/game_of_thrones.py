...
from visualization.plot.pyvis.plot_pyvis import plot_graph


def plot_got_with_barnes_hut():
    got_graph = load_got_graph()
    plot_graph(got_graph, output_filename='got_barnes_hut.html', barnes_hut=True)


def plot_got_panel():
    got_graph = load_got_graph()
    plot_graph(got_graph, panel=True, output_filename='../../../presentations/presentation_one/got_panel.html')
...
def plot_got_lpa():
    graph = load_got_graph(integer_index=True)
    graph = apply_model_to_graph(graph, LabelPropagation())
    plot_graph(graph, output_filename='got_lpa.html')
